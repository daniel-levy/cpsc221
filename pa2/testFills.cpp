...
TEST_CASE("Pinwheel test 1"){
    
	PNG img;
    img.readFromFile(RAINTESTIMAGE);
	
    animation anim;
    anim = filler::fillCustomBFS(img, RAINX, RAINY, RAINTOLERANCE, RAINFRAMEFREQ);
	
    PNG result = anim.write("images/bfspin.gif");
    result.writeToFile("images/bfspin.png"); 
	
    REQUIRE(true); 
}

TEST_CASE("Pinwheel test 2"){

    PNG img;
    img.readFromFile(RAINTESTIMAGE);

    animation anim;
    anim = filler::fillPinDFS(img, RAINX, RAINY, RAINTOLERANCE, RAINFRAMEFREQ);
	
    PNG result = anim.write("images/dfspin.gif");
    result.writeToFile("images/dfspin.png");
    REQUIRE(true);
}
